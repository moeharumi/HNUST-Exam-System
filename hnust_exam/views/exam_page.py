...
class ExamPage(QWidget):
    ...
    def setup_exam(self, exam: Exam, file_path: str) -> None:
        ...
        self._exam_start_ts = datetime.now()
    ...
    def _report_score(self, score_pct: float) -> None:
        """上报成绩到统计服务（静默，不阻塞）."""
        try:
            if not self._device_id or not self.exam:
                return
            exam_name = os.path.splitext(os.path.basename(self.exam_file_path))[0]
            duration = EXAM_TIME_SECONDS - self.remaining_time
            types = ",".join(sorted({q.q_type for q in self.exam.questions}))
            send_submit_score(self._device_id, exam_name, score_pct, duration, types)
        except Exception:
            pass
